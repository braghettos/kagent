...
type Config struct {
	...
	// PassthroughURL, when set, is the base URL (e.g. "http://127.0.0.1:18789")
	// that requests not handled by the shim (/acp, /health, /healthz) are
	// reverse-proxied to. Used to keep an in-sandbox HTTP UI (e.g. the
	// OpenClaw Control UI) reachable when the shim owns the ingress port.
...
func (c *Config) Validate() error {
	...
	if c.PassthroughURL != "" {
		u, err := url.Parse(c.PassthroughURL)
		if err != nil || u.Scheme == "" || u.Host == "" {
			return fmt.Errorf("invalid passthrough URL %q (want e.g. http://127.0.0.1:18789)", c.PassthroughURL)
		}
	}
	return nil
}
