...
var mcpServerToolControllerLog = ctrl.Log.WithName("mcpserver-tool-controller")
...
func (r *MCPServerToolController) recordEvent(ctx context.Context, req ctrl.Request, eventtype, reason, action, note string, args ...interface{}) {
	...
	if err := r.Client.Get(ctx, req.NamespacedName, server); err != nil {
		if !apierrors.IsNotFound(err) {
			mcpServerToolControllerLog.V(1).Info("unable to fetch MCPServer for event recording", "error", err.Error())
		}
		return
	}
