...
func (r *RemoteMCPServerController) recordEvent(ctx context.Context, req ctrl.Request, eventtype, reason, action, note string, args ...interface{}) {
	...
	if err := r.Client.Get(ctx, req.NamespacedName, server); err != nil {
		if !apierrors.IsNotFound(err) {
			remoteMCPServerControllerLog.V(1).Info("unable to fetch RemoteMCPServer for event recording", "error", err.Error())
		}
		return
	}
