...
func (r *ModelConfigController) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	err := r.Reconciler.ReconcileKagentModelConfig(ctx, req)
	if err != nil {
...
func (r *ModelConfigController) recordEvent(ctx context.Context, req ctrl.Request, eventtype, reason, action, note string, args ...interface{}) {
	...
	if err := r.Client.Get(ctx, req.NamespacedName, mc); err != nil {
		if !apierrors.IsNotFound(err) {
			modelConfigControllerLog.V(1).Info("unable to fetch ModelConfig for event recording", "error", err.Error())
		}
		return
	}
