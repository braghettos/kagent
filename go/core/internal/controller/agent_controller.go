...
func (r *AgentController) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	err := r.Reconciler.ReconcileKagentAgent(ctx, req)
	if err != nil {
...
func (r *AgentController) recordEvent(ctx context.Context, req ctrl.Request, eventtype, reason, action, note string, args ...interface{}) {
	...
	if err := r.Client.Get(ctx, req.NamespacedName, agent); err != nil {
		if !apierrors.IsNotFound(err) {
			agentControllerLog.V(1).Info("unable to fetch Agent for event recording", "error", err.Error())
		}
		return
	}
