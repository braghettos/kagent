...
func (e *KAgentExecutor) Execute(ctx context.Context, reqCtx *a2asrv.RequestContext, queue eventqueue.Queue) error {
	...
	for adkEvent, adkErr := range r.Run(ctx, userID, sessionID, content, runConfig) {
		...
		// Record GenAI token usage on the events that carry it (no-op unless
		// OTEL_METRICS_ENABLED). Output combines candidate + reasoning tokens.
		if um := adkEvent.UsageMetadata; um != nil {
			...
			telemetry.RecordTokenUsage(ctx, e.modelName, e.providerName, input, output)
		}
